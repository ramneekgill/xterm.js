...
interface IDemoAddon<T extends AddonType> {
  ...
  ctor: T extends "attach"
    ...
    ? typeof SearchAddon
    : T extends "serialize"
    ? typeof SerializeAddon
    : T extends "webLinks"
    ? typeof WebLinksAddon
    : T extends "unicode11"
    ? typeof Unicode11Addon
    : T extends "unicodeGraphemes"
    ? typeof UnicodeGraphemesAddon
    : T extends "webgl"
    ? typeof WebglAddon
    : never;
...
if (document.location.pathname === "/test") {
  ...
  document
    .getElementById("create-new-window")
    .addEventListener("click", createNewWindowButtonHandler);
...
function createTerminal(): void {
  ...
  setTimeout(async () => {
    ...
    const res = await fetch("/terminals?cols=" + term.cols + "&rows=" + term.rows, {
      method: "POST",
    });
...
function initOptions(term: Terminal): void {
  const blacklistedOptions = [
    ...
    "termName",
    "cols",
    "rows", // subsumed by "size" (colsRows) option
    // Complex option
    "documentOverride",
    "linkHandler",
    "logger",
  ...
  options
    ...
    .forEach((o) => {
      switch (typeof term.options[o]) {
        ...
        default:
          if (
            Object.keys(stringOptions).indexOf(o) === -1 &&
            numberOptions.indexOf(o) === -1 &&
            booleanOptions.indexOf(o) === -1
          ) {
  ...
  Object.keys(stringOptions).forEach((o) => {
    if (o === "colsRows") {
      ...
      html += `<div class="option"><label>${o} <select id="opt-${o}">${stringOptions[o]
        .map((v) => `<option ${v === selectedOption ? "selected" : ""}>${v}</option>`)
  ...
  Object.keys(stringOptions).forEach((o) => {
    const input = document.getElementById(`opt-${o}`) as HTMLInputElement;
    addDomListener(input, "change", () => {
      console.log("change fawjhefna", o, input.value);
      let value: any = input.value;
      console.log("option is: ", o);
...
function initAddons(term: Terminal): void {
  ...
  Object.keys(addons).forEach((name: AddonType) => {
    ...
    addDomListener(checkbox, "change", () => {
      if (name === "image") {
        if (checkbox.checked) {
          const ctorOptionsJson =
            document.querySelector<HTMLTextAreaElement>("#image-options").value;
...
function updateTerminalSize(): void {
  const width = autoResize
    ? "100%"
    : (
        term._core._renderService.dimensions.css.canvas.width + term._core.viewport.scrollBarWidth
      ).toString() + "px";
...
function underlineTest(): void {
  ...
  function showSequence(id: number, name: string): string {
    ...
    return (
      `${u(id)}4:${id}m - ${name}\x1b[4:0m`.padEnd(33, " ") +
      `${u(id)}${alphabet} ${numbers} 汉语 한국어 👽\x1b[4:0m`
    );
  }
...
function sgrTest(): void {
  ...
  for (const e of entries) {
    term.writeln(
      `\x1b[0m\x1b[${e.ps}m ${e.ps.toString().padEnd(2, " ")} ${e.name.padEnd(
        maxNameLength,
        " "
      )} - ${testString}\x1b[0m`
    );
...
function addAnsiHyperlink(): void {
  ...
  term.writeln(`\nAdjacent links:`);
  term.writeln(
    "\x1b]8;;https://github.com\x07GitHub\x1b]8;;https://xtermjs.org\x07\x1b[32mxterm.js\x1b[0m\x1b]8;;\x07"
  );
  ...
  term.writeln("╚════╝");
  term.write(
    "\x1b[3A\x1b[1C\x1b]8;;https://xtermjs.org\x07xter\x1b[B\x1b[4Dm.js\x1b]8;;\x07\x1b[2B\x1b[5D"
  );
}
...
function addGraphemeClusters(): void {
  ...
  term.writeln(
    "\u{1F3CB}\u{FE0F}=\u{1F3CB}\u{1F3FE}\u{200D}\u{2640}\u{FE0F} [person lifting weights emoji; woman lighting weights, medium dark]"
  );
  term.writeln(
    "\u{1F469}\u{1F469}\u{200D}\u{1F393}\u{1F468}\u{1F3FF}\u{200D}\u{1F393} [woman; woman student; man student dark]"
  );
  ...
  term.writeln(
    "<E\u0301\ufe0fg\ufe0fa\ufe0fl\ufe0fi\ufe0f\ufe0ft\ufe0fe\u0301\ufe0f> [Égalité (using separate acute) emoij_presentation]"
  );
}
...
function addOverviewRuler(): void {
  ...
  term.registerDecoration({
    ...
    overviewRulerOptions: { color: "#ef2929" },
  });
  term.registerDecoration({
    marker: term.registerMarker(3),
    overviewRulerOptions: { color: "#8ae234" },
  });
  term.registerDecoration({
    marker: term.registerMarker(5),
    overviewRulerOptions: { color: "#729fcf" },
  });
  term.registerDecoration({
    marker: term.registerMarker(7),
    overviewRulerOptions: { color: "#ef2929", position: "left" },
  });
  term.registerDecoration({
    marker: term.registerMarker(7),
    overviewRulerOptions: { color: "#8ae234", position: "center" },
  });
  term.registerDecoration({
    marker: term.registerMarker(7),
    overviewRulerOptions: { color: "#729fcf", position: "right" },
  });
  term.registerDecoration({
    marker: term.registerMarker(10),
    overviewRulerOptions: { color: "#8ae234", position: "center" },
  });
  term.registerDecoration({
    marker: term.registerMarker(10),
    overviewRulerOptions: { color: "#ffffff80", position: "full" },
  });
}
  ...
  if (decorationStressTestDecorations) {
    ...
    for (const x of [
      ...
      for (let y = 0; y < t.buffer.active.length; y++) {
        ...
        decorationStressTestDecorations.push(
...
(console as any).image = (source: ImageData | HTMLCanvasElement, scale: number = 1) => {
  ...
  console.log(
    ...
    `${
      dim.style
    }background: url(${canvas.toDataURL()}); background-size: ${sw}px ${sh}px; background-repeat: no-repeat; color: transparent;`
  );
  console.groupCollapsed("Zoomed");
  console.log(
    `%c${dim.string}`,
    `${getBox(sw * 10, sh * 10).style}background: url(${canvas.toDataURL()}); background-size: ${
      sw * 10
    }px ${
      sh * 10
    }px; background-repeat: no-repeat; color: transparent; image-rendering: pixelated;-ms-interpolation-mode: nearest-neighbor;`
  );
  console.groupEnd();
};
...
function addVtButtons(): void {
  ...
  const buttonSpecs: {
    [key: string]: { label: string; description: string; paramCount?: number };
  } = {
...
function initImageAddonExposed(): void {
  ...
  term.element.addEventListener("click", (ev: MouseEvent) => {
    ...
    const pos = term._core._mouseService!.getCoords(
      ev,
